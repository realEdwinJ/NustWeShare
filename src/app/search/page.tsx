      ...
      {!trimmed ? (
        <Card className="mt-6">
          <CardContent className="py-10 text-center">
            <p className="text-sm text-muted-foreground">Try: <Link prefetch={false} href="/search?q=MCI511S" className="underline hover:text-foreground">MCI511S</Link> · <Link prefetch={false} href="/search?q=PLU411S" className="underline">PLU411S</Link> · <Link prefetch={false} href="/search?q=COA511S" className="underline">COA511S</Link></p>
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid gap-3 sm:grid-cols-2">
                  {data.modules.map((m) => (
                    <Link prefetch={false} key={m.code} href={`/modules/${m.code.toLowerCase()}`} className="group rounded-xl border p-4 hover:bg-accent">
              ...
              <CardContent>
                <div className="space-y-2">
                  {data.programmes.map((p) => (
                    <Link prefetch={false} key={p.code} href={`/search?q=${encodeURIComponent(p.code)}`} className="flex items-center justify-between rounded-xl border px-3 py-3 hover:bg-accent">
                      <span>
